...
function PharmacyLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await api.post('/pharmacy/login', { email, password });
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#070b16] relative overflow-hidden px-4">
      ...
      <div className="relative z-10 bg-[#0f1729]/80 backdrop-blur-xl border border-teal-500/10 rounded-2xl shadow-2xl p-8 w-full max-w-md">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-white">Pharmacy Login</h2>
          <p className="text-sm text-slate-400 mt-1">Login to manage your pharmacy</p>
        </div>
        ...
        <p className="text-center text-sm text-slate-400 mt-6">
          New pharmacy?{' '}
          <Link to="/pharmacy/register" className="text-teal-400 font-medium">
            Register here
          </Link>
        </p>
